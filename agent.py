...
class ReviewWorkflowState(MessagesState):
    ...
    review_status: Annotated[
        Optional[str], "Current review status: 'pending', 'approved', 'needs_revision'"
    ]
...
def create_main_agent(
    model: Any, tools: List[Any], prompt: str, name: str = "main_agent"
):
    """
    ...
    """
    return create_react_agent(model=model, tools=tools, prompt=prompt, name=name)
...
{feedback_context}"""
    ...
    formatted_prompt = default_prompt.format(
        task=task, feedback_context=feedback_context
    )
...
def create_review_workflow() -> StateGraph:
    ...
    workflow.add_conditional_edges(
        "review_agent", should_continue, {"main_agent": "main_agent", "END": END}
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Final message: {result['messages'][-1].content if result['messages'] else 'No messages'}"
    )
